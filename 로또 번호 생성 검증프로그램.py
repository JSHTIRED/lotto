...
for I in printlist2:
    ...
    if 0<len(i&RR)<=2 and len(i&A)<3 and len(i&B)<3 and len(i&C)<3 and len(i&D)<3 and len(i&
        ...
        KKK)<2 and len(i&III)<2 and len(i&QQQ)<3 and len(i&WWW)<3 and len(i&ZZZ)<3 and len(i&QWERR)<3 and 2<=len(i&UND)<=4 and 16.67<=(sum(i)/6)<=30 and 2<=len(i&BBB)<=4 and 1<=len(i&DDD)<=2 and 2<=len(i&MMS)<=4 and len(i&FFF)<2 and len(i&HHH)<2:
        # 로또번호 추출 방법
            UO+=1
            # 조건을 만족하는 경우의 수
            if len(i&TW)==3:
